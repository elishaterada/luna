...
struct EditorSkin: Codable, Equatable, Identifiable, Sendable {
    ...
    let hasAudio: Bool
    var muted = true
    /// Nil follows the terminal theme; optional so 0.4.0 catalogs remain readable.
    var backgroundRGB: UInt32?

    static func backgroundRGB(from color: NSColor) -> UInt32? {
    ...
    func backgroundColor(light: Bool) -> NSColor {
        let rgb = backgroundRGB ?? (light ? 0xF7F8FA : 0x292A2B)
...
struct SkinConfiguration: Codable, Equatable, Sendable {
    ...
    var selected: EditorSkin? { skins.first { $0.id == selectedID } }
    mutating func normalize() {
        ...
        for index in skins.indices where (skins[index].backgroundRGB ?? 0) > 0xFFFFFF {
            skins[index].backgroundRGB = nil
        }
    }
...
final class SkinLibrary: ObservableObject {
    ...
    private var previewPreparation: Task<Void, Never>?

    init(root: URL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        .appendingPathComponent("Luna", isDirectory: true)
        ...
        if !loadFailed {
            let photos = configuration.skins.filter { $0.kind == .photo }
            let root = root
            previewPreparation = Task { [weak self] in
                let work = Task.detached(priority: .utility) {
                    var failures: [UUID: String] = [:]
                    for skin in photos {
                        let folder = root.appendingPathComponent(skin.id.uuidString)
                        let preview = folder.appendingPathComponent("poster.png")
                        if !FileManager.default.fileExists(atPath: preview.path) {
                            do { try SkinImporter.writePhotoPreview(from: folder.appendingPathComponent(skin.filename), to: preview) }
                            catch { failures[skin.id] = "\(skin.name): \(error.localizedDescription)" }
                        }
                    }
                    return failures
                }
                let failures = await work.value
                guard let self else { return }
                let remaining = self.configuration.skins.compactMap { failures[$0.id] }
                if !remaining.isEmpty {
                    self.errorMessage = "Could not restore photo transparency: " + remaining.joined(separator: "; ")
                }
    ...
    func url(for skin: EditorSkin) -> URL { folder(for: skin.id).appendingPathComponent(skin.filename) }
    func posterURL(for skin: EditorSkin) -> URL {
        let png = folder(for: skin.id).appendingPathComponent("poster.png")
        if skin.kind == .photo, FileManager.default.fileExists(atPath: png.path) { return png }
        return folder(for: skin.id).appendingPathComponent("poster.jpg")
    }
    func waitForPhotoPreviews() async { await previewPreparation?.value }
    ...
    @discardableResult func add(_ source: URL) async throws -> EditorSkin {
    ...
    nonisolated static func failure(_ message: String) -> NSError {
        NSError(domain: "Luna.Skin", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
    }
}

private enum SkinImporter {
    static func photoPreview(from source: URL) throws -> CGImage {
    ...
    static func writePhotoPreview(from source: URL, to destination: URL) throws {
        try writePreview(photoPreview(from: source), to: destination, photo: true)
    }
    private static func writePreview(_ image: CGImage, to destination: URL, photo: Bool) throws {
        let data = NSMutableData()
        let type = photo ? UTType.png : UTType.jpeg
        guard let output = CGImageDestinationCreateWithData(data, type.identifier as CFString, 1, nil) else {
            throw SkinLibrary.failure("Could not create the skin preview.")
        }
        CGImageDestinationAddImage(output, image, photo ? nil : [kCGImageDestinationLossyCompressionQuality: 0.9] as CFDictionary)
    ...
    static func copy(_ source: URL, to root: URL) async throws -> EditorSkin {
        ...
        do {
            ...
            if isVideo {
                ...
                poster = try photoPreview(from: destination)
            }
            try writePreview(poster, to: folder.appendingPathComponent(isVideo ? "poster.jpg" : "poster.png"), photo: !isVideo)
            try Task.checkCancellation()
            try FileManager.default.moveItem(at: folder, to: root.appendingPathComponent(id.uuidString))
            return EditorSkin(id: id, name: source.deletingPathExtension().lastPathComponent, filename: filename,
