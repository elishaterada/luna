...
/// Owns playback and pointer tracking independently of editor rendering and SwiftUI.
final class SkinBackgroundView: NSView {
    ...
    func configure(library: SkinLibrary, preview: Bool = false) {
        ...
        let skin = (preview || config.enabled) ? config.selected : nil
        let posterURL = skin.map { library.posterURL(for: $0) }
        if selectedID != skin?.id || loadedPosterURL != posterURL {
            stop(); selectedID = skin?.id; loadedPosterURL = posterURL
    ...
    private func refreshAppearance() {
        layer?.backgroundColor = config.selected?.backgroundColor(light: EditorPreferences.isLight).cgColor
